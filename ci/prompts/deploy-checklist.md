...
You are running in a CI pipeline. Your task is to generate a deployment verification checklist from the diff between `main` and `deploy-production`.

Output ONLY the checklist markdown. Do not wrap it in code fences, do not add explanations before or after the checklist.
