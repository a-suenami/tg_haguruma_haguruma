...
- [ ] ログイン画面を署名付きURLで保護する
  - 目的: `/login` を内部ユーザーのみアクセス可能にする
  - 方針: Ruler から署名付きURLを発行し、署名なしアクセスは拒否
  - 実装方針:
    - Rails の `MessageVerifier` を使って署名付きURLを生成・検証
    - Ruler の管理画面にログインURL発行機能を追加
    - ログインコントローラーの `before_action` で署名・有効期限を検証
    - 署名なし or 期限切れ → 404 を返す（存在自体を隠す）
  - 検討事項:
    - 有効期限の長さ（短すぎると不便、長すぎるとリスク）
    - ログイン画面のパス自体も変更するか（併用で多層防御）
    - Ruler 上のUI（ボタン一つで発行？一覧管理？）
