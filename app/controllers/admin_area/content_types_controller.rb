...
module AdminArea
  class ContentTypesController < AdminArea::ApplicationController
    ...
    def create
      ...
      @content_type.tenant_id = T.must(Tenant.current_id)
    ...
    def update
      if T.must(@content_type).update(content_type_params)
        redirect_to admin_area_content_type_path(@content_type),
